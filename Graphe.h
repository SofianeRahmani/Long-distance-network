...
*/
    struct Ponderations
    {
        ...
        Ponderations(float dur, float c): duree(dur), cout(c){};
    };
 ...
 */
    ...
    public:
        ...
        std::string reqNomSommet(size_t sommet) const;

        bool sommetExiste(size_t sommet) const;

        bool arcExistePourNumero(size_t depart, size_t arrive) const;

        Graphe inverse() const;

        void ajouterSommet (size_t sommet);

        bool invariant() const;
        void explorer_dfs_en_partant_de(size_t numero, std::stack<size_t>& resultat, std::vector<bool>& visite) const;
        std::stack<size_t> explorer_dfs() const;
        size_t taille() const;
